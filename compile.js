...
function createConfiguration() {
    return {
        language: 'Solidity',
        sources: {
            'DogCollectionTest.sol': {
                content: fs.readFileSync(path.resolve(__dirname, 'contracts', 'DogCollectionTest.sol'), 'utf8')
            },
            'Migrations.sol': {
                content: fs.readFileSync(path.resolve(__dirname, 'contracts', 'Migrations.sol'), 'utf8')
            }
        },
        settings: {
...
function getImports(dependency) {
    ...
    switch (dependency) {
        case 'DogTest.sol':
            return {contents: fs.readFileSync(path.resolve(__dirname, 'contracts', 'DogTest.sol'), 'utf8')};
        default:
